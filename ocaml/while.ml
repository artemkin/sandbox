...
let rec eval_expr : type a. (string, int) Hashtbl.t -> a expr -> a =
  fun vars expr -> match expr with
    | Var name -> Hashtbl.find vars name
    | Int_lit n -> n
    | Bool_lit n -> n
    | Add (a, b) -> (eval_expr vars a) + (eval_expr vars b)
    | Sub (a, b) -> (eval_expr vars a) - (eval_expr vars b)
    | Mul (a, b) -> (eval_expr vars a) * (eval_expr vars b)
    | Div (a, b) -> (eval_expr vars a) / (eval_expr vars b)
    | Lt  (a, b) -> (eval_expr vars a) < (eval_expr vars b)
    | Gt  (a, b) -> (eval_expr vars a) > (eval_expr vars b)
    | And (a, b) -> (eval_expr vars a) && (eval_expr vars b)
    | Or  (a, b) -> (eval_expr vars a) || (eval_expr vars b)
...
let rec eval vars statement =
  match statement with
  | Assign (name, expr) ->
    Hashtbl.replace vars name (eval_expr vars expr)
  | If (cond, s1, s2) ->
    if (eval_expr vars cond) then (eval vars s1) else (eval vars s2)
  | While (cond, statement) ->
    while (eval_expr vars cond) do
      eval vars statement
    done
  | Seq statements -> List.iter (eval vars) statements
...
let apply_op exprs op =
  let create_expr (Expr (a_type, a)) (Expr (b_type, b)) =
    ...
    | (`Add | `Sub | `Mul | `Div | `Lt | `Gt | `And | `Or), _, _, _, _ -> assert false
  in
